...
|---|---|
| "An unauthenticated user sees only login and the unsupported/denied status area where relevant." | `home.spec.ts` → *"an unauthenticated user sees only login and a status message"* |
| "An authenticated user sees logout and the app shell." | `login.spec.ts` → step *"AC: authenticated user sees logout and the app shell"* (after a real, completed Entra login) |
...
`login.spec.ts` drives Entra's real hosted sign-in UI rather than mocking
